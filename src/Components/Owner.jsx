...
// import { FaEdit, FaTrash, FaEye, FaEyeSlash, FaSignOutAlt, FaUser, FaUsers, FaCalendar, FaBlog, FaBars, FaTimes, FaPlus, FaTachometerAlt } from 'react-icons/fa';
...
//   // ✅ Toggle expanded state for dashboard events
//   const toggleDashboardEventExpanded = (eventId) => {
//     setExpandedDashboardEvents(prev => ({
//       ...prev,
//       [eventId]: !prev[eventId]
//     }));
//   };
...
//   // ✅ Truncate text for dashboard events (shorter version)
//   const truncateTextShort = (text, maxLines = 2) => {
//     if (!text) return '';
//     const lines = text.split('\n');
//     if (lines.length <= maxLines) return text;
//     return lines.slice(0, maxLines).join('\n') + '...';
...
//       formData.append("author", user?.username || "Owner"); // Add author name

//       const res = await axios.post(`${API_BASE}/blogs`, formData, {
//         headers: { "Content-Type": "multipart/form-data" },
//       });

//       // Add the new blog with proper image path and author
//       const newBlog = {
//         ...res.data,
...
//             <p><strong>Author:</strong> {user?.username || "Owner"}</p>
//           </div>
//           <div className="form-actions">
//             <button onClick={handleSubmit} className="btn-primary" disabled={loading}>
...
//         const pendingBlogs = blogs.filter(blog => !blog.published);
//         const totalBlogViews = blogs.reduce((total, blog) => total + (blog.views || 0), 0);
...
//           const end = new Date(event.endDate || event.end_date);
//           return {
//             ...event,
...
//         const activeEvents = updatedEvents.filter(event => event.isActive && event.published).length;
//         const inactiveEvents = updatedEvents.filter(event => !event.isActive && event.published).length;
//         const pendingEvents = updatedEvents.filter(event => !event.published).length;
//         const adminUsers = admins.length;
//         const bloggerUsers = bloggers.length;
//         const readers = JSON.parse(localStorage.getItem('readers')) || [];

//         return (
//           <div className="dashboard-stats" style={{ overflowX: 'hidden' }}>
//             <h2>Dashboard Overview</h2>
//             {backgroundLoading && (
//               <div className="loading subtle" style={{ opacity: 0.7 }}>
//                 Refreshing data...
//               </div>
//             )}
...
//                   <div>Published: {publishedBlogs.length}</div>
//                   <div>Pending: {pendingBlogs.length}</div>
//                 </div>
//               </div>
//               <div className="stat-box">
//                 <h3>{allComments.length}</h3>
//                 <p>Total Comments</p>
//                 <div className="stat-details">Recent: {recentComments.length}</div>
//               </div>
//               <div className="stat-box">
//                 <h3>{totalBlogViews}</h3>
//                 <p>Total Blog Views</p>
//                 <div className="stat-details">Recent: {totalBlogViews}</div>
//               </div>
//               <div className="stat-box">
//                 <h3>{adminUsers + bloggerUsers}</h3>
//                 <p>Total Users</p>
//                 <div className="stat-details">
//                   <div>Admins: {adminUsers}</div>
//                   <div>Bloggers: {bloggerUsers}</div>
//                 </div>
//               </div>
//               <div className="stat-box">
//                 <h3>{readers.length}</h3>
//                 <p>Total Readers</p>
//                 <div className="stat-details">Recent: {readers.length}</div>
//               </div>
//               <div className="stat-box">
//                 <h3>{events.length}</h3>
//                 <p>Total Events</p>
//                 <div className="stat-details">
//                   <div>Active: {activeEvents}</div>
//                   <div>Inactive: {inactiveEvents}</div>
//                   <div>Pending: {pendingEvents}</div>
//                 </div>
//               </div>
//             </div>
//             <h3 className='events-title'>Active Events</h3>
//             <div className="events-grid-dashboard">
//               {updatedEvents.filter(e => e.isActive && e.published).length > 0 ? (
//                 updatedEvents.filter(e => e.isActive && e.published).map(event => {
//                   const displayDescription = truncateTextShort(event.description || 'No description available', 2);
                  
//                   return (
//                     <div key={event.id} className="event-dashboard-card">
//                       {event.image && (
...
//                           <strong>Dates:</strong> {new Date(event.startDate || event.start_date).toLocaleDateString()} 
//                           {' to '} 
...
//                   <h3>No active events currently</h3>
//                 </div>
//               )}
//             </div>
//           </div>
...
//                           <strong>Author:</strong> {blog.author || user?.username || "Unknown"}
//                         </div>
...
//             className={activeView === 'Dashboard' ? 'sidebar-btn active' : 'sidebar-btn'}
//           >
...
const Owner = () => {
  ...
  const handleAddBlog = async () => {
    ...
    try {
      ...
      const authorName = user?.username || 'Owner';
                  ...
                  return (
                    <div className="blog-square-card" key={blog.id}>
                      ...
                      <div className="square-card-content">
                        ...
                        {/* Display author information */}
                        <div className="blog-author">
                          <strong>Author:</strong> {blog.author || user?.username || "Owner"}
                        </div>
